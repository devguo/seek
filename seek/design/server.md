...
##### 1.4 使用 Go 开发全部逻辑, 包括业务模块和性能敏感模块(战斗、寻路等). 通过 graceful restart 实现服务热更新;
...
##### 3.5 raidsvr (副本服)
负责跨服副本玩法, 具备客户端接入能力. 内部并发模型:

- **goroutine-per-connection**: 每个客户端连接 reader + writer goroutine, 收包 → 投递副本 goroutine channel
- **副本 goroutine**: 按副本 ID hash 分配到固定 goroutine, 副本内战斗逻辑串行, 无需加锁
...
- **代码热更**: Go 二进制通过 graceful restart 实现. 新二进制启动 → 继承监听端口 → 旧进程排干连接后退出. 支持灰度(先更新部分实例观察)
...
- **同步 RPC**: 在 goroutine 中阻塞等待 channel 返回, 不占用额外线程
...
##### 5.1 开发语言
- **Go**: 全部服务代码(网络 IO / RPC / 业务逻辑 / 战斗)
- Go 原生 goroutine + channel 替代线程池和协程, `net` 包替代 asio
- 热更新通过 graceful restart(新二进制启动 → 旧进程排干 → 下线)
...
- **指标采集**: Prometheus + Grafana
  - 关键指标: QPS、延迟(P50/P99)、错误率、连接数、CPU/内存、goroutine 数
  - 业务指标: 在线人数、副本完成率、匹配耗时
- **告警**: 错误率突增、延迟 P99 超过阈值、服务实例不可用

##### 5.5 并发模型
- Go goroutine: 每个连接/每个请求一个 goroutine, 阻塞调用不占用线程
- channel: 消息队列由 buffered channel 实现
- `sync.WaitGroup` + `context.Context` 管理 goroutine 生命周期
- 同步 RPC 直接在 goroutine 中阻塞等待 channel 返回, 无需额外协程调度器
...
##### 7.1 goroutine 模型
所有服务进程(网关/gamesvr/raidsvr/netbus/dbsvr)共用此模型:

- **每个 TCP 连接一个 reader goroutine + writer goroutine**: reader 收包 → 投递到对应队列; writer 从写队列取包 → 发送
- **每个玩家一个 goroutine** (gamesvr): 处理该玩家的所有业务消息, 通过 buffered channel 接收消息. 保证同一玩家消息串行, 无需加锁
- **worker goroutine 池** (raidsvr): 战斗副本按副本 ID hash 分配到固定 goroutine, 副本内逻辑串行
- **GOMAXPROCS**: 设置为 CPU 核数, 多个 goroutine 多路复用在线程上
- gamesvr 仅开 TCP 端口, 内部通信; raidsvr 按需开 TCP/UDP/WS 端口, 客户端接入

##### 7.2 玩家绑定策略
- 同一 playerId 的消息通过 buffered channel 投递到同一个 player goroutine, 避免并发冲突
- `sync.Map` 或 sharded map 管理 playerId → channel 映射
- 世界数据/大地图等共享状态使用 `sync.RWMutex` 或 sharded lock

##### 7.3 消息队列

```mermaid
flowchart LR
    Conn[TCP 连接] -->|reader goroutine| Dispatch{按 playerId<br/>hash 分配}
    Dispatch -->|消息| Q1[player goroutine 1<br/>buffered chan]
    Dispatch -->|消息| Q2[player goroutine 2<br/>buffered chan]
    Dispatch -->|...| QN[player goroutine N<br/>buffered chan]
    Q1 --> W1[writer goroutine]
    Q2 --> W2[writer goroutine]
    QN --> WN[writer goroutine]
    W1 --> Conn
```
...
| 模块 | 说明 |
|------|------|
| 网络 IO | Go `net` 包, TCP/UDP/KCP/WS 多协议支持. goroutine-per-connection, 按配置开启协议. gamesvr 只开 TCP(内部通信), 网关全开 |
| 消息队列 | buffered channel 实现, 收包 → 按 playerId hash → player goroutine channel → 处理 → writer goroutine 发包 |
| 并发模型 | goroutine + channel + sync 原语, GOMAXPROCS=CPU 核数 |
| 消息分发 | Protobuf Descriptor 做 key, 注册 Handler. Command/Handler 模式, Handler 编排多模块调用. 所有服务共用 | 
| RPC 框架 | 基于 protobuf + channel, 同步/异步, 超时/重试/熔断 |
| dirsvr 注册 | 启动注册 + 心跳维持 |
| 日志/指标 | 结构化 JSON 日志, Prometheus 指标 |
| UUID 生成 | 统一 ID 生成器, 防止不同服务实例间 ID 重复 |
| 配置加载 | 启动加载 + reload 热更 |
| 生命周期管理 | 统一启动/停止/维护接口, graceful restart |
...
- **网关**: 独立代码(客户端接入逻辑差异大), 复用网络层和消息校验模块
- **dbsvr**: 独立代码(数据同步逻辑), 复用网络层和 RPC 框架

##### 9.3 功能开关示例

```
# 以 gamesvr 模式启动
./server --mode=game --tcp=:9000

# 以 raidsvr 模式启动
./server --mode=raid --tcp=:9001 --udp=:9002 --ws=:9003
```
...
func (h UseItemHandler) Handle(req *pb.UseItemReq, p *Player) {
    item := p.Inventory().Remove(req.ItemId, req.Count)
    p.Buff().Apply(item.Effect)
    p.Quest().OnItemUsed(req.ItemId)
}

dispatcher.Register(&pb.UseItemReq{}, UseItemHandler{})
```
...
**热更流程**: 灰度选择实例 → 启动新二进制(继承原端口) → 旧进程排干 → 验证(监控错误率) → 全量推广或回滚. 配置热更走 reload 指令, 不重启进程
...
##### 9.7 配置管理
- 配置表使用 Excel/CSV 编辑, 导出为 JSON 或 protobuf 二进制
